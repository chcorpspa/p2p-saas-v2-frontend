...
export default function OrdersPage() {
  ...
  // Fetch history
  const { data: historyData } = useQuery<{ orders: ActiveOrder[]; total: number }>({
  ...
  const orders = tab === 'active' ? activeOrders : (historyData?.orders ?? []);
        ...
        <div className="p-4 border-b border-border space-y-3">
          <div className="flex gap-2">
            <button
              ...
              className={`flex-1 text-sm py-1 rounded ${tab === 'active' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'}`}
            >
              Activas
            </button>
            <button
              onClick={() => setTab('history')}
              className={`flex-1 text-sm py-1 rounded ${tab === 'history' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'}`}
            >
              Historial
            </button>
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {orders.length === 0 && (
            <div className="p-6 text-center text-muted-foreground text-sm">
              {tab === 'active' ? 'Sin órdenes activas' : 'Sin historial'}
            </div>
          )}
